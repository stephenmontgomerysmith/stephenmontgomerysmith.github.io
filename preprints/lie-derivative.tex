...
\newcommand\setinvertibledualquat{\mathbb D}
\newcommand\setdualquat{\mathfrak d}
\newcommand\setunitdualquat{\mathbb{SD}}
...
\title{Using Lie derivatives with dual quaternions in robotics}
...
We set $\setinvertibledualquat$ for the set of invertible dual quaternions (that is, $A +\epsilon B$ where $A \ne 0$), $\setdualquat$ for the set of dual quaternions, $\setunitdualquat$ for the set of unit dual quaternions, and $\setpuredualquat$ for the set of pure dual quaternions.

A pose of the form
\begin{equation}
\label{pose action}
\bm r \mapsto Q \bm r \overline Q + \bm t,
\end{equation}
...
\end{equation}
Composition of poses corresponds to multiplication of unit dual quaternions.  If $\bm r$ is a 3-vector, and $\bm s$ is the image of $\bm r$ under the action of the pose $\eta = Q + \epsilon B$, then
\begin{equation}
...
A \emph{twist} is the pair of vectors $(\bm w, \bm v)$ that describe the change of pose in the moving reference frame, where $\bm w$ is the angular velocity, and $\bm v$ is the translational velocity, that is, if $\bm r$ is a constant vector, then
\begin{equation}
\label{ode twist}
\frac d{dt} (Q \bm r \overline Q + \bm t) = Q (\bm w \times \bm r + \bm v) \overline Q + \bm t .
\end{equation}
Then a twist can be represented by a pure dual quaternion:
\begin{equation}
...
\begin{gather}
|\eta|^2 = \overline{\eta} \eta = \eta \overline\eta = |A|^2 + 2 \epsilon (B \cdot A) ,\\
\label{norm}
...
We remark that since the exponential of a dual quaternion \cite{wang-et-al} is defined by
\begin{equation}
\label{exponential}
\exp(\theta) = \sum_{k=0}^\infty \frac{\theta^k}{k!},
\end{equation}
and if $\theta$ is a pure dual quaternion, then by equation~\eqref{approx normalize} we have
\begin{equation}
\label{exp-normal}
...
\node[left] at (0,1) {$r$};
\node[below] at (0.72,1.7) {$\theta$};
\node[below] at (0.33,1.9) {$\psi$};
\node at (8,-3) {$\theta$};
...
\draw[->] (-3,4) -- (-3,5) node[above] {$\boldsymbol n$};
\draw[-] (-3,1) -- (-3,2) node[above] {\minibox{Axis of\\rotation}};
...
In this section, we consider only a single cable attached to a point $\tilde{\bm r}$ on the end effector.  Let us suppose that the cable attaches via an assembly, which is free to rotate about an axis parallel to the unit vector $\bm n$, and passing through the point $\bm s$.  Attached to the assembly, at a fixed distance $h_0$ from the fixed point $\bm s$, by a rod perpendicular to $\bm n$, is the center of a pulley of radius $r$, which rotates in the plane containing the axis of rotation and the point on the end effector $\tilde{\bm r}$.  A cable passes over the pulley in the $\bm n$ direction from the center of the pulley at the point $\bm a$.  See Figure~\ref{pulley}.
...
\begin{equation}
\tilde{\bm r} = Q \bm r \overline Q + 2 B \overline Q = (Q \bm r + 2 B) \overline Q .
\end{equation}
Now, if $\theta = \tfrac12 \bm a + \tfrac12 \epsilon \bm b$ is a pure dual quaternion, then
\begin{equation}
...
\end{gather}
Remembering $\overline{\bm a} = -\bm a$, and that $\bm a \bm r - \bm r \bm a = 2 \,\bm a \times \bm r$, we obtain
\begin{equation}
\label{lie deriv point}
\liederiv_\theta(\tilde{\bm r}) = Q (\bm a \times \bm r + \bm b) \overline Q.
\end{equation}
And if $\psi_2 = \tfrac12 \bm c + \tfrac12 \epsilon \bm d$ is another pure dual quaternion, then
\begin{equation}
\liederiv\psi \liederiv_\theta(\tilde{\bm r}) = Q(\bm c \times (\bm a \times \bm r + \bm b)) \overline Q .
\end{equation}
...
\end{equation}
where $\mathsf I$ is the $(3 \times 3)$ identity matrix, and $\starop{\bm r}$ is the \emph{Hodge star operator} of $\bm r$ \cite{wiki0}:
\begin{equation}
...
We first discuss hte `big-Oh' notation in more detail.  In \cite{montgomery-smith-functional-calculus}, we show how to define the `functional calculus' of dual quaternions.  That is, given a continuously differentiable function $f:\mathbb C \to \mathbb C$, satisfying $f(\bar z) = \overline{f(z)}$, we can make sense of $f(\eta)$ for any dual quaternion $\eta = A + \epsilon B$.  First we can define $f$ on quaternions by realizing that any unit vector $\bm n$ satisfies $\bm n^2 = -1$.  Next, if $B$ is decomposed as $B_1 + \bm b_2$, where $B_1$ commutes with $A$, and $\bm b_2$ is a pure quaternion that anti-commutes with $\imagpart(A)$, and setting
\begin{gather}
...
\end{equation}
This means that for any functions $f$ and $g$, that $\|f(A + \epsilon B)\| \le \|g(A + \epsilon B)\|$ for all quaternions $B$ if and only if $|f(A)| \le |g(A)|$, $|f_x(A)| \le |g_x(A)|$, $|f_{iy}(A)| \le |g_{iy}(A)|$, and $|h_f(A)| \le |h_g(A)|$.  In particular, the comparision of size does depend upon which characteristic length $l$ is used.
...
\begin{equation}
\liederiv_\psi \bm h = Q(\bm a \times \bm r_0 + \bm b) \overline Q .
\end{equation}
...
\bibitem{montgomery-smith-et-al} Stephen Montgomery-Smith and Cecil Shy, An introduction to using dual quaternions to study kinematics, preprint.
...
\item $\setinvertibledualquat$: the set of invertible dual quaternions.
