...
\begin{multline}
\label{hessian}
\left.\frac{\partial^2}{\partial\theta^2} \tilde g(\eta(1+\theta)) \right|_{\theta = 0} \\
= \left[\tfrac12 (\liederiv_i \liederiv_j g(\eta) + \liederiv_j \liederiv_i g(\eta)) \right]_{1\le i,j\le 6}.
\end{multline}
...
The results of this section may be found in \cite{montgomery-smith}.  We have seen similar results in other places, but the ones we have found seem to have errors.
...
%&= \psi \cdot ((\mathsf M\varphi) \rtimes \varphi - \varphi \ltimes (\mathsf M\varphi)) \\
%&= 2 \psi \cdot ((\mathsf M\varphi) \rtimes \varphi)) .
%\end{aligned}
%\end{equation}
\end{proof}

%From equation~\eqref{rtimes} we immediately obtain the following.
...
&= 
\tfrac12(
Q \bm a \bm r_0 \overline Q - Q \bm r_0 \bm a \overline Q \\
&\phantom{{}={}(Q} + 2 B \bm a \overline Q + 2 Q \bm b \overline Q - 2 B \bm a \overline Q ) \\
...
\begin{equation}
\begin{aligned}
\liederiv_\psi (\bm g \cdot \bm h) &= \tilde{\bm g} \cdot (\bm a \times \bm r_0 + \bm b) \\
&= 2 \psi \cdot (\bm r_0 \times \tilde{\bm g} + \epsilon \tilde{\bm g}) .
\end{aligned}
\end{equation}
\end{proof}
...
+ \liederiv_\varphi \liederiv_\psi \mathsf L \cdot \mathsf M_0\liederiv_\varphi \mathsf L \\
& \phantom{{}={}} - \liederiv_\psi \liederiv_\varphi \mathsf L \cdot \mathsf M_0\liederiv_\varphi \mathsf L \\
